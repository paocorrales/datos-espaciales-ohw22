...
temperature %>% 
  .[lev == 1000 & lon %between% c(275, 315) & lat %between% c(-60, -10)] %>% 
...
```{r echo=FALSE, fig.height=6}
observaciones <- fread("https://raw.githubusercontent.com/eliocamp/r-clima/master/datos/observaciones_smn.csv") %>% 
  group_by(station) %>% 
  summarise(tmax_media = mean(tmax, na.rm = TRUE))

estaciones <- fread("https://raw.githubusercontent.com/eliocamp/r-clima/master/datos/estaciones_smn.csv") 
...
1. `r emo::ji("computer")` Entrá a este proyecto de RStudio Cloud  
...
4.`r emo::ji("save")` Hacé click en "Save a permanent copy" (arriba a la derecha)
...
background-image: url(slides/img/background)
